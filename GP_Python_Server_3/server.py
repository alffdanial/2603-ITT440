...
cursor.execute("SELECT points FROM scoreboard WHERE user='python_user3'")
...
while True:
    ...
    try:
        ...
        if request == "reset":
            ...
            """, (points, current_time))
            db.commit()
            response = f"python_user3 score reset to 0 at {current_time}"
        else:
            cursor.execute("""
            SELECT points
            FROM scoreboard
            ...
            response = f"python_user3 score: {result[0] if result else 0}"
