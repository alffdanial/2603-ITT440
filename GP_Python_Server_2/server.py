...
cursor.execute("SELECT points FROM scoreboard WHERE user='python_user2'")
...
while True:
    ...
    try:
        ...
        if request == "reset":
            ...
            """, (points, current_time))
            db.commit()
            response = f"python_user2 score reset to 0 at {current_time}"
        else:
            cursor.execute("""
            SELECT points
            FROM scoreboard
            ...
            response = f"python_user2 score: {result[0] if result else 0}"
